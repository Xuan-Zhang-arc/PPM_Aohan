...
global.model.AB<-ppm(AB.multitype~s(elevation_im)+s(slope_im)+s(aspect_im)+s(vis_in_im)+s(vis_out_im)+landforms_recla_im+s(river_im)+soil_recla_im+s(geo_rock_im)+s(geo_earth_im)+s(precipitation_im)+s(temperature_im), use.gam=TRUE)
...
param.importance.AB
#              params.AB    aic.imp
# 1   landforms_recla_im 0.00000000
# 2         s(aspect_im) 0.00180018
# 3      s(elevation_im) 0.96479648
# 4      s(geo_earth_im) 0.98679868
# 5       s(geo_rock_im) 0.03520352
# 6  s(precipitation_im) 1.00000000
# 7          s(river_im) 1.00000000
# 8          s(slope_im) 0.03730373
# 9    s(temperature_im) 0.03870387
# 10        s(vis_in_im) 1.00000000
# 11       s(vis_out_im) 0.96269627
# 12       soil_recla_im 0.53985399


# 6.2.3 models and goodness of fit
model.AB <- ppm(AB.multitype~s(elevation_im)+s(geo_earth_im) +s(precipitation_im) +s(river_im) +s(vis_in_im)+s(vis_out_im), use.gam=TRUE) 
...
AB.RR <- data.frame(R=seq(100, 15000, by=100))
